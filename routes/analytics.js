...
router.get('/overview', async (req, res) => {
  try {
    ...
    const trend = Object.entries(monthMap).sort(([a], [b]) => a.localeCompare(b)).map(([month, data]) => ({ month, ...data }))
    const streetMap = {}
    all.forEach(c => { streetMap[c.street] = (streetMap[c.street] || 0) + 1 })
    const topStreets = Object.entries(streetMap).sort(([, a], [, b]) => b - a).slice(0, 10).map(([street, count]) => ({ street, count }))
    res.json({
