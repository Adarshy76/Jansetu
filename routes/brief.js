...
router.get('/weekly', auth.adminOnly, async (req, res) => {
  try {
    ...
    if (worstWard.score < 40) recommendations.push({ priority: 'CRITICAL', title: `Immediate Action — ${worstWard.ward}`, desc: `GIS score critically low at ${worstWard.score}/100. ${worstWard.total} unresolved complaints. Deploy team immediately.` })
    ...
    recommendations.push({ priority: 'INFO', title: `Replicate ${bestWard.ward} Model`, desc: `Best performing ward with GIS ${bestWard.score}/100. ${bestWard.resolved}/${bestWard.total} resolved. Study and scale their approach.` })
